...
template<typename T>
// reutrns (max_area, max_closest area)
pair<int, int> f(BinTree<T> const& t){
	if(t.empty()) return {0,0};
	else{
		int mca = 0, ma = 0; 
		auto [ma_r, mca_r] = f(t.right());
		auto [ma_l, mca_l] = f(t.left());
		if(t.root() != 'B')
			mca = mca_r + mca_l + 1;
		ma = max(max(ma_r, ma_l), mca);
		return { ma, mca };
	}
}

template<typename T>
int max_area(BinTree<T> const& t){
	return f(t).first;
}
