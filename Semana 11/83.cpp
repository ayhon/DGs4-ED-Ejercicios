...
/*
 * Analizador léxico del lenguaje Calculon++
 */
...
/*
 * Analizador sintáctico del lenguaje Calculon++
 */
...
/*
 * Función que lee una sentencia del lenguaje Calculon++ de la entrega
 * y devuelve su árbol sintáctico.
 */
...
int simple_eval(string op, int a, int b){
	int res = 0;
	switch(op[0]){
		case '*':
			res = a * b;
...
bool isLeaf(BinTree<string> const& t){
	return !t.empty() && t.right().empty() && t.left().empty();
}

int eval(BinTree<string> const& t, map<string, int> const& vars) {
	if(isLeaf(t)){
		string val = t.root();
		if(vars.count(val))
			return vars.at(val);
		...
		string op = t.root();
		int a = eval(t.left(), vars);
		int b = eval(t.right(), vars);
		return simple_eval(op, a, b);
	}
}
...
bool tratar_caso() {
	...
	for(int i = 0; i < n; ++i){
		t = parse(cin);
		string varname = t.left().root();
		int value = eval(t.right(), vars);
		vars[varname] = value;
	}

	for(auto const& [varname, value] : vars)
		printf("%s = %d\n", varname.c_str(), value);
	cout << "---\n";
