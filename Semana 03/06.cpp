 ...
 * Estudiante 2: Jaime Jacobo Romo 
 * Estudiante 3: Jinqing Cai 
 *
 */
//@ </answer>
...
void ListLinkedSingle::reverse_segment(int index, int length) {
    /// Inicio es el nodo antes del primer nodo a procesar
    /// Puede ser nullptr si el primer elemento a procesar es el primero de la lista
    Node* inicio = nullptr; // If inicio == nullptr → actualizar head
    Node* curr = head;
    ...
    if(inicio != nullptr){
        /// Si no es el primer elemento de la lista el que giramos
        /// Entonces tiene un elemento previo
        inicio->next->next = curr;
		inicio->next = prev;
    }
    else{
        /// Se giro el primer elemento de la lista
        /// Hay que actualizar el head
        head->next = curr;
        head = prev;
    }
}
