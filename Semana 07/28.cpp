...
struct Datos{
	...
	Datos() {}
	Datos(bool b) :  cumple(b) {}
};

template <typename T>
Datos<T> f(BinTree<T> const& tree){
	if(tree.empty()) return {true};

	Datos<T> rv,
		  dl = f(tree.left()),
		  dr = f(tree.right());
	rv.cumple = dl.cumple && dr.cumple;

	if(!tree.left().empty()){
		rv.cumple = rv.cumple && dl.maxi < tree.root();
		rv.mini = dl.mini;
	}
	else rv.mini = tree.root();
	if(!tree.right().empty()){
		rv.cumple = rv.cumple && tree.root() < dr.mini;
		rv.maxi = dr.maxi;
	}
	else rv.maxi = tree.root();
	return rv;
}

template <typename T>
bool esBusqueda(BinTree<T> const& tree){
	return f(tree).cumple;
}
...
bool resuelveCaso(){
	...
	if(op == 'P'){
		...
		cout << (esBusqueda<string>(tree)? "SI" : "NO") << '\n';
	}
	else{
		BinTree<int> tree = read_tree<int>(cin);
		cout << (esBusqueda<int>(tree)? "SI" : "NO") << '\n';
	}
