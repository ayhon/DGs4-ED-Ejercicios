 ...
 */
  ...
  */
  ...
  //@ <answer>
  // Nombre y Apellidos: Jaime Romo González
  //@ </answer>
...
BinTree<int> reconstruir
(vector<int> const& preorden, vector<int> const& inorden,
    int& idxP, int iniI, int finI) {
    if (iniI >= finI) return BinTree<int>();
    else {
        int root = preorden[idxP++];
        int idx;

        for (idx = iniI; idx < finI; ++idx)
            if (inorden[idx] == root) break;

        return {
            reconstruir(preorden, inorden, idxP, iniI, idx),
            root,
            reconstruir(preorden, inorden, idxP, idx + 1, finI)
        };
    }
}
...
BinTree<int> reconstruir(vector<int> const& preorden, vector<int> const& inorden) {
    int idx = 0;
    return reconstruir(preorden, inorden, idx, 0, inorden.size());
}
