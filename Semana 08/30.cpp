...
private:
  struct Node {
    ...
    std::pair<bool,T> lower_bound(Node * root, T const& elem) const;
...
template <typename T>
std::pair<bool,T> SetTree<T>::lower_bound(Node * root, T const& elem) const {
	if (root == nullptr) return { false, T() };

	if (elem == root->elem) return { true, root->elem };
    else if (elem < root->elem){
		auto [found, res] = lower_bound(root->left, elem);
		if (!found) res = root->elem;
        return {true, res};
	}
    else return lower_bound(root->right, elem);
}
