...
BinTree<int> resolver(vector<int> const& preorden, int &idx, int const& top){
	if(idx >= (int)preorden.size()     || 
	  (top != -1 && preorden[idx] > top))
		return {};
	int const& root = preorden[idx];
	idx++;
	BinTree<int> izq = resolver(preorden, idx, root),
			     der = resolver(preorden, idx, top);
	return { izq, root, der };
}

BinTree<int> constr_ABB_con_preorden(vector<int> const& preorden){
	int idx = 0;
	return resolver(preorden, idx, -1);
}
...
bool resuelveCaso(){
	...
	BinTree<int> t = constr_ABB_con_preorden(preorden);
	cout << t << '\n';
