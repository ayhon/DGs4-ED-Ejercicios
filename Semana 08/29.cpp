...
template <typename T> 
int SetTree<T>::count_interval(Node* root, T const& lower, T const& upper) {
	...
	if (root->elem >= lower && root->elem <= upper){
        izq = count_interval(root->left, lower, upper) + 1;
        der = count_interval(root->right, lower, upper);  
    } 
    else if (root->elem < lower) {
        der = count_interval(root->right, lower, upper);
    }
    else {
        izq = count_interval(root->left, lower, upper);
    }
